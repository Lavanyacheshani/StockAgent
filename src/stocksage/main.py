...
def train():
    ...
    try:
        StockSage().crew().train(n_iterations=int(sys.argv[2]), filename=sys.argv[3], inputs=inputs)
    except Exception as e:
...
def replay():
    ...
    """
    try:
        StockSage().crew().replay(task_id=sys.argv[2])
    except Exception as e:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m stocksage.main <command> [arguments]")
        print("Commands:")
        print("  run        - Execute full stock analysis pipeline")
        print("  test       - Run test mode (first task only)")
        print("  train      - Train mode (requires iterations and filename)")
        print("  replay     - Replay a previous run (requires task_id)")
        sys.exit(1)
    ...
    if command == "run":
        ...
        if len(sys.argv) < 4:
            print("Error: train requires 2 arguments: <iterations> <filename>")
            sys.exit(1)
        train()
    elif command == "replay":
        if len(sys.argv) < 3:
            print("Error: replay requires 1 argument: <task_id>")
            sys.exit(1)
        # Support both 'replay <task_id>' and 'replay --run_id <task_id>'
        if sys.argv[2] == "--run_id" and len(sys.argv) > 3:
            # We shift the task_id to be at sys.argv[2]
            # temporarily so the replay() function finds it
            sys.argv[2] = sys.argv[3]
        replay()
    else:
        print(f"Unknown command: {command}")
        print("Available commands: run, test, train, replay")
        sys.exit(1)
