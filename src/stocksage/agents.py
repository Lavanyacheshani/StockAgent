...
def create_analysis_agent(
    ...
    config: dict[str, str | int] = AGENT_META_CONFIG,
    ...
    default_backstory: str = (
        "You are an investment analyst tasked with"
        " integrating financial and sentiment"
        " data for stock selection."
    ),
    default_instructions: str = """
        Integrate quantitative financial data
        with market sentiment analysis to create
        comprehensive stock assessments.
        Ensure you provide detailed analysis
        for each stock, combining financial
        metrics and sentiment data.
    """,
) -> Agent:
...
def create_fact_agent(
    ...
    default_backstory: str = (
        "You are a financial analyst tasked with analyzing quantitative data for stock selection."
    ),
    default_instructions: str = """
...
def create_justification_agent(
    ...
    default_backstory: str = (
        "You are an investment justification analyst"
        " tasked with providing detailed rationale"
        " for stock selections."
    ),
    default_instructions: str = """
        Provide detailed justification for the
        most promising stocks based on fundamental
        and sentiment analysis.
        Ensure you include a clear rationale for
        each stock selection, supported by
    ...
    return Agent(
        ...
        llm=llm,
        verbose=verbose,
        instructions=justification_agent_config.get("instructions", default_instructions),
        config=config,
        step_callback=callback,
    )
...
def create_optimization_agent(
    ...
    default_backstory: str = (
        "You are an investment optimizer tasked"
        " with refining stock selections based"
        " on risk-reward profiles."
    ),
    default_instructions: str = """
        Critically analyze and refine the stock
        selections based on risk-reward profiles.
        Ensure you provide an optimized selection
        of stocks with risk assessment and
    ...
    Args:
        ...
        verbose (bool, optional): Whether to enable verbose output. Defaults to VERBOSE.
        allow_delegation (bool, optional): Whether agent can delegate tasks. Defaults to False.
        async_execution (bool, optional): Whether to execute tasks
            asynchronously. Defaults to False.
        temperature (float, optional): LLM temperature setting. Defaults to 0.3.
        default_role (str, optional): Agent's role if not in config.
            Defaults to "Investment Optimizer".
        default_goal (str, optional): Agent's goal if not in config.
        default_backstory (str, optional): Agent's backstory if not in config.
        default_instructions (str, optional): Agent's instructions if not in config.
    ...
    return Agent(
        ...
        instructions=optimization_agent_config.get("instructions", default_instructions),
        verbose=verbose,
        llm=llm,
        allow_delegation=allow_delegation,
        tools=[
...
def create_portfolio_manager_agent(
    ...
    default_backstory: str = (
        "You are an experienced portfolio manager tasked with overseeing the investment process."
    ),
) -> Agent:
    """
...
def create_recommendation_agent(
    ...
    default_backstory: str = (
        ...
        " tasked with providing actionable"
    ...
    """
    recommendation_agent_config = agents_config.get("recommendation_agent", {})
    return Agent(
...
def create_sentiment_agent(
    ...
    default_backstory: str = (
        "You are an expert in analyzing public sentiment and news for stock selection."
    ),
    default_instructions: str = """
        ...
        Focus on creating a MULTI-DIMENSIONAL
        sentiment analysis that goes beyond
        just financial news.
    """,
) -> Agent:
    """
...
def create_synthesizer_agent(
    ...
    default_backstory: str = (
        "You are a senior portfolio strategist who"
        " excels at synthesizing diverse analyses"
        " into coherent investment strategies."
    ),
    default_instructions: str = """
        For each stock:
    ...
    Args:
        ...
        default_role (str, optional): Agent's role if not in config.
            Defaults to "Investment Synthesizer".
        default_goal (str, optional): Agent's goal if not in config.
        default_backstory (str, optional): Agent's backstory if not in config.
        default_instructions (str, optional): Agent's instructions if not in config.
...
def create_thesis_agent(
    ...
    default_backstory: str = (
        "You are an investment thesis developer"
        " tasked with creating detailed investment"
        " theses for selected stocks."
    ),
    default_instructions: str = """
        Develop comprehensive investment theses
        for the top 5 selected stocks.
        Ensure you provide a detailed investment
        thesis for each stock with a compelling
...
def create_terminator_agent(
    ...
    default_backstory: str = (
        "You are a process terminator tasked with"
        " ensuring all resources are properly"
        " released and the process terminates"
        " cleanly."
    ),
    default_instructions: str = """
    ...
    Args:
        ...
        temperature (float, optional): LLM temperature setting. Defaults to 0.5.
        default_role (str, optional): Agent's role if not in config.
            Defaults to "Process Terminator".
        default_goal (str, optional): Agent's goal if not in config.
        default_backstory (str, optional): Agent's backstory if not in config.
        default_instructions (str, optional): Agent's instructions if not in config.
