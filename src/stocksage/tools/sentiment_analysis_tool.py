...
class SentimentAnalysisInput(BaseModel):
    ...
    search_query: str | None = Field(
        None,
        description=(
            "Custom search query to use instead of default."
            " Use this to target specific aspects like"
            " 'product reviews', 'CEO reputation', etc."
        ),
    )
...
class SentimentAnalysisTool(BaseTool):
    ...
    def __init__(self, serper_api_key: str | None = None, **kwargs):
        """
    ...
    # @traceable
    def _run(
        self, ticker: str, days: int | None = 30, search_query: str | None = None
    ) -> dict[str, Any]:
        """
            ...
            processed_news.append(
                {
                    ...
                    "date": self.get_deterministic_date(ticker, item.get("title", ""), days),
        ...
        if processed_news:
            overall_sentiment_score = round(total_sentiment_score / len(processed_news), 2)
        else:
            # Deterministic fallback sentiment based on ticker
            overall_sentiment_score = self.get_deterministic_sentiment(ticker, "fallback")
        ...
        analyst_ratings = self.get_deterministic_analyst_ratings(overall_sentiment_score)

        # Get company name
        company_name = self.get_company_name_for_ticker(ticker)

        # Add search context to the result
        search_context = search_query if search_query else f"{ticker} stock financial news analysis"
    ...
    def fetch_stock_news(
        self, ticker: str, search_query: str | None = None
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.info(
                ...
                f" for {ticker} with query: {query}"
            )
            return news_items

        except Exception as e:
            logger.error(f"Error fetching news data for {ticker} with query '{query}': {e}")
    ...
    # @traceable
    def get_deterministic_social_media_mentions(self, ticker: str, news_count: int) -> int:
        """
    ...
    # @traceable
    def get_deterministic_analyst_ratings(self, sentiment_score: float) -> dict[str, int]:
        """
    ...
    def get_fallback_news_data(
        ...
        for i, headline in enumerate(headlines):
            snippet_theme = search_query if search_query else f"{industry} business strategy"
            news_items.append(
                {
                    "title": headline,
                    "snippet": (
                        f"{company_name} ({ticker}) shows promising"
                        f" developments in its {snippet_theme} with"
                        " recent announcements and growing market"
                        " presence."
                    ),
