...
class StockSymbolRequest(BaseModel):
    ...
    limit: int | None = Field(default=None, description="Maximum number of symbols to return")
...
class StockSymbolFetcherTool(BaseTool):
    ...
    description: str = "Fetches stock symbols from various sources like S&P 500, NASDAQ-100, etc."
    ...
    async def _arun(
        ...
        Args:
            ...
            custom_symbols (List[str], optional): Custom list of
                stock symbols when source is 'custom'.
                Defaults to [].
            limit (int, optional): Maximum number of symbols to
        ...
        if source is None or source.lower() in ["s&p 500", "s&p500", "sp 500"]:
            source = "sp500"
    ...
    def _run(
        ...
        Args:
            source (str, optional): Source of stock symbols
                ('sp500', 'nasdaq100', 'dow30', 'all', 'custom').
                Defaults to "sp500".
            custom_symbols (List[str], optional): Custom list of
                stock symbols when source is 'custom'.
                Defaults to [].
        ...
        source = source if source is not None else "sp500"
