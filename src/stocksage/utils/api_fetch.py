        ...
        if html:
            ...
            for _i, table in enumerate(tables):
                ...
                ticker_cols = [
                    ...
                    for col in columns
                    if any(
                        name in str(col).lower() for name in ["ticker", "symbol", "trade", "code"]
                    )
                ]
                if ticker_cols:
...
async def get_alpha_vantage_data(
    ...
    function: str | None = "TIME_SERIES_INTRADAY",
    ...
    Args:
        ...
        session (aiohttp.ClientSession): Active aiohttp session for making HTTP requests
        function (str, optional): Alpha Vantage API function to
            call. Defaults to "TIME_SERIES_INTRADAY".
