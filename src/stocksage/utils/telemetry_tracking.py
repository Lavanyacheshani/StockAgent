import asyncio
import os
from datetime import UTC, datetime  # Use timezone-aware objects
...
def _build_langsmith_client() -> Client | None:
    """Build the LangSmith client only when the API key is available."""
...
def verify_langsmith_setup() -> bool:
    ...
    client = langsmith_client or _build_langsmith_client()
    if client is None:
        print("❌ Could not create LangSmith client — LANGSMITH_API_KEY not set")
        return False

    # Initialize client and verify connection
    try:
        client.create_project(os.getenv("LANGSMITH_PROJECT"), upsert=True)
